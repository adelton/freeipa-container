...
dig +short @$( kubectl get -o=jsonpath='{.spec.clusterIP}' service freeipa-server-service ) $IPA_SERVER_HOSTNAME | grep .
dig +short @$( kubectl get -o=jsonpath='{.spec.clusterIP}' service freeipa-server-service ) -t srv _ldap._tcp.${IPA_SERVER_HOSTNAME#*.} | grep .
...
kubectl exec pod/freeipa-replica -- dig +short $IPA_SERVER_HOSTNAME | grep .
kubectl exec pod/freeipa-replica -- dig +short $IPA_REPLICA_HOSTNAME | grep .
